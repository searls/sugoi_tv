...
struct ProgramGuideServiceTests {
  ...
  func liveProgram() {
    ...
    let current = entries.liveProgram(at: Date(timeIntervalSince1970: 2500))
    #expect(current?.title == "Show B")

    let first = entries.liveProgram(at: Date(timeIntervalSince1970: 1500))
    #expect(first?.title == "Show A")

    let before = entries.liveProgram(at: Date(timeIntervalSince1970: 500))
    #expect(before == nil)
  }
  ...
  func upcomingPrograms() {
    ...
    let upcoming = entries.upcomingPrograms(after: Date(timeIntervalSince1970: 2500), limit: 2)
  ...
  func vodAvailable() {
    ...
    let vod = entries.vodAvailable(before: Date(timeIntervalSince1970: 5000))
    #expect(vod.count == 1)
